...
| 5 | Does dedup work across DOI, title, hash, AND downstream AE inventory? | **DOI + normalized-title yes; hash = stored-for-integrity (not a dedup key); AE-inventory = local probe** | `article_references` has `UNIQUE(doi)` (duplicate DOIs impossible in buffer); `search_runner` dedups on insert by normalized DOI then normalized title. `pdf_sha256` is **stored for integrity only** — it is *not* used in any dedup `WHERE`/`SELECT` (honest correction). Handoff is idempotent via `handoff_log UNIQUE(reference_id)`; `probe_pdf_against_article_eater` checks the local inventory (documented substitute for real AE). |
| 6 | Are tests portable from a clean checkout, or do they need sibling repos / absolute paths? | **Portable** | `gap_extractor` no longer `sys.exit`s. **`abstract_triage.py` and `gap_extractor.py` now locate `atlas_shared` via install→`$KA_ATLAS_SHARED_SRC`→sibling and honor `$KA_CONSTITUTIONS`/`$TRACK2_MECHANISMS`** (review fix — they previously hardcoded the sibling path / ignored the env). Both data files (`question_constitutions_starter.json`, `mechanisms.json`) are **bundled at `task3/fixtures/`**, so Task 2/3 no longer needs a sibling `Knowledge_Atlas` checkout and the counts are stable (51/51 offline) in a fresh checkout — not a sibling-dependent subset. `validate_task1.py` locates `atlas_shared` the same way and SKIPs cleanly (exit 0) if absent. No `/private/tmp` paths. |
...
| 10 | Are all documentation claims strictly supported by runnable evidence? | **Yes** | Every claim here maps to a command. Suites: Task 1 `42/42`, Task 2+3 `51/51` (offline) / `55/55` (`T2_LIVE=1`), chain `9/9`. Stubs (AE, scidownl) are labeled as stubs, not integrations. |

## Reproduce everything
```bash
# Article_Finder root
python3 -m pytest task3/tests_task2_task3.py -q          # pytest-collectable, passes
python3 task3/tests_task2_task3.py                       # 51/51 offline
T2_LIVE=1 python3 task3/tests_task2_task3.py             # 55/55 incl. real abstract + real OA PDF
