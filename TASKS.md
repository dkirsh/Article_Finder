...
| T16 | **Efficient κ-pilot pack for Stephan** (URGENT — blocks his restart email's ZIP): run a second independent extraction route over the frozen 20-paper R2 pack (content hash acd779c4... must not change), diff vs pack candidate projections, emit disagreement-first question ordering + per-field Wilson stop-rule harness in the review interface; blind presentation preserved; new manifest/receipt per R2→R3 precedent ("scientific inputs changed: false"). Zero Stephan legacy data to migrate (verified 2026-09-11). | 2026-09-11 | David's ruling: efficient version inside the pilot; sprint amendment in New_VR_Platform students/ |
